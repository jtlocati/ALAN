...
def main() -> None:
    ...
    try:
        while True:
            ...
            dealerCards = [d.label for d in view.dealer]
            playerCards = [d.label for d in view.player]

            dealerCount = sum(BJ_COUNTS.get(rankCards(l), 0) for l in set(dealerCards))
            playerCount = sum(BJ_COUNTS.get(rankCards(l), 0) for l in set(playerCards))
            ...
            cv2.putText(frame, f"pot >= ${potTotal}   cards {view.total_cards}",
